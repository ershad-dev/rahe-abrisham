...
/**
 * @component Login
 * @description مدیریت ورود کاربران با قابلیت "مرا به خاطر بسپار" و ناوبری ریسپانسیو.
 */

import { reactive, ref, onMounted } from 'vue'
import { useRouter, useRoute } from 'nuxt/app'

// --- تعاریف و وضعیت‌ها (States) ---

const router = useRouter()
const route = useRoute()

/** @interface LoginForm ساختار داده‌های فرم ورود */
interface LoginForm {
  email: string;
  password: string;
  rememberMe: boolean;
...
// --- متدها و منطق‌ها (Methods) ---

/** بازیابی ایمیل ذخیره شده از localStorage */
onMounted(() => {
  ...
  <div class="min-h-screen flex items-center justify-center bg-[#f4f7fa] bg-[url('~/assets/images/login-bg.png')] bg-cover bg-center p-4 dir-ltr font-sans">
    ...
    <div :class="{'shake': isShaking}" class="w-full max-w-[700px] md:h-[380px] bg-white rounded-2xl border border-gray-100 shadow-xl flex flex-col md:flex-row animate-[fadeIn_0.6s_ease-out] overflow-visible">
      ...
      <div class="flex md:hidden w-full border-b border-gray-100 overflow-hidden rounded-t-2xl bg-gray-50/50">
        <NuxtLink 
          to="/register" 
          class="flex-1 py-4 flex flex-col items-center gap-1 transition-all duration-300"
          :class="route.path === '/register' ? 'bg-white border-b-2 border-[#2b2bb5] text-[#2b2bb5]' : 'text-gray-400 opacity-60'"
        >
        ...
        <NuxtLink 
          to="/login" 
          class="flex-1 py-4 flex flex-col items-center gap-1 transition-all duration-300"
          :class="route.path === '/login' ? 'bg-white border-b-2 border-[#2b2bb5] text-[#2b2bb5]' : 'text-gray-400 opacity-60'"
        >
          <img src="~/assets/images/plane.png" class="w-5 h-5" />
      ...
      <form @submit.prevent="handleLogin" class="flex-1 flex flex-col justify-center py-6 px-6 md:px-10 md:pr-2">
        ...
        <div class="mb-3 text-right">
          ...
          <input 
            ...
            type="email" 
            dir="ltr"
            class="w-full h-10 rounded-full border border-gray-200 px-4 text-[13px] outline-none focus:border-[#0a0a5e] bg-[#ebebeb]/40 focus:bg-white transition-all" 
          />
        </div>
        ...
        <button 
          type="submit" 
          :disabled="isLoading" 
          class="w-full h-10 bg-[#0b0b54] text-white text-sm font-bold rounded-lg hover:bg-[#15158a] transition-all active:scale-95 shadow-md disabled:opacity-70 disabled:cursor-not-allowed"
        >
