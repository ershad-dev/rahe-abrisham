  ...
  <div class="min-h-screen flex items-center justify-center bg-[#f4f7fa] bg-[url('~/assets/images/login-bg.png')] bg-cover bg-center p-4 dir-ltr font-sans">
    <div :class="{'shake': isShaking}" class="w-full max-w-[700px] md:h-[380px] mt-[50px] rounded-2xl border border-gray-100 shadow-xl flex flex-col md:flex-row animate-[fadeIn_0.6s_ease-out] overflow-visible text-right ">
      ...
      <div class="relative w-[90px] border-l border-gray-50 hidden md:flex flex-col items-center justify-center gap-8">
        <div class="absolute left-[-2px] top-[185px] w-1 h-14 bg-[#0b0b54] rounded-full transition-all duration-300"></div>
      ...
      <form @submit.prevent="handleLogin" class="flex-1 flex flex-col justify-center py-6 px-6 md:px-10 md:pr-2">
        ...
        <div class="flex justify-between items-center text-[10px] mb-4 px-2 text-right">
          <NuxtLink to="/forgetPassword" class="text-[#0a0a5e] hover:underline font-bold">
            فراموشی رمز عبور
          </NuxtLink>
          <label class="flex items-center gap-2 cursor-pointer select-none group">
            <span class="text-[#0a0a5e] font-bold group-hover:text-blue-800 transition-colors">مرا به خاطر بسپار</span>
            <div class="relative flex items-center justify-center">
              <input type="checkbox" v-model="form.rememberMe" class="peer hidden">
              <div class="w-4 h-4 border-2 border-[#0b0b54] rounded-[4px] bg-white transition-all 
                          peer-checked:bg-[#0b0b54] peer-checked:border-[#0b0b54] 
...
// دیتای پیش‌فرض در صورت نبودن در LocalStorage
...
onMounted(() => {
  if (process.client) {
    // مقداردهی اولیه دیتابیس مجازی اگر وجود نداشته باشد
    if (!localStorage.getItem('all_mock_users')) {
      localStorage.setItem('all_mock_users', JSON.stringify(defaultMockUsers))
    }

    const savedEmail = localStorage.getItem('userEmail')
    const rememberState = localStorage.getItem('rememberMeState')
    
    if (savedEmail) form.email = savedEmail
    if (rememberState === 'true') form.rememberMe = true
  }
})
...
const handleLogin = async () => {
  ...
  if (process.client) {
    // خواندن دیتای کاربران از LocalStorage (این همان لیستی است که در فراموشی رمز آپدیت می‌شود)
    const storedUsers = JSON.parse(localStorage.getItem('all_mock_users') || JSON.stringify(defaultMockUsers))
    
    // جستجو بر اساس ایمیل و پسورد (پسورد جدید در اینجا چک می‌شود)
    const foundUser = storedUsers.find((u: any) => u.email === form.email && u.password === form.password)
