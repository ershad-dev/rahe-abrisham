    ...
    <div class="cards-wrapper flex flex-col gap-[35px] md:gap-[45px] w-full items-center px-4">
      ...
      >
        <img 
          ...
          alt="icon" 
          class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[350px] md:w-[400px] opacity-50 pointer-events-none z-0"
        >
        ...
        <div class="relative z-10 w-full md:w-[48%] flex flex-col justify-between">
          <ul class="flex flex-col gap-[12px] items-center md:items-start">
            <li v-for="(info, i) in item.details" :key="i" class="flex items-center text-[15px] md:text-[17px] whitespace-nowrap">
              ...
              <span class="font-black ml-[6px] text-[#BFFFFA]">{{ info.label }} :</span>
              <span class="text-white font-bold">{{ info.value }}</span>
            </li>
          </ul>
        ...
        <div class="relative z-10 w-full md:w-[48%] flex flex-col justify-between items-center md:items-end text-center md:text-left">
          <p class="text-[17px] md:text-[19px] font-bold mb-4 md:mb-0 text-[#BFFFFA]">
            زمان تحویل کالا : <span class="text-white">{{ item.deliveryTime }}</span>
          </p>
          
          <div class="flex flex-col gap-2 md:items-end">
            <p class="text-[16px] md:text-[18px] font-bold text-[#BFFFFA]">
              قیمت هر سهم به دلار : <span class="text-white">${{ item.pricePerShare }}</span>
            </p>
            <p class="text-[16px] md:text-[18px] font-bold text-[#BFFFFA]">
              مقدار سهم : <span class="text-white">{{ item.shareAmount }}</span>
            </p>
          </div>
